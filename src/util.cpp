...
const char * const MOGCOIN_CONF_FILENAME = "mogcoin.conf";
const char * const MOGCOIN_PID_FILENAME = "mogcoind.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "mogcoind -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MogCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MogCoin
    // Mac: ~/Library/Application Support/MogCoin
    // Unix: ~/.mogcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MogCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/MogCoin";
#else
    // Unix
    return pathRet / ".mogcoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", MOGCOIN_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", MOGCOIN_PID_FILENAME)));
}
