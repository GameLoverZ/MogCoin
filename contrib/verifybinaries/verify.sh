...
###   mogcoincore.org and mogcoin.org and compares them.
...
WORKINGDIR="/tmp/mogcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://mogcoincore.org"
HOST2="https://mogcoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="mogcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'mogcoin-',
...
if [ $? -ne 0 ]; then
   echo "mogcoin.org failed to provide signature file, but mogcoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "mogcoin.org and mogcoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the MogCoin Core binary release signing key installed?"
   fi
